...
//-->]]></script>
	...
	<xsl:template name="give-header">
		<h1>Installation instructions for '<xsl:value-of select="$title" />' Version <xsl:for-each select="mod:mod-version">
		...
		<div id="modDisclaimer">
			...
			<p>This MOD was designed for phpBB<xsl:value-of select="mod:installation/mod:target-version" />and may not function as stated on other phpBB versions. MODs for phpBB3.0 will <strong>not</strong> work on phpBB2.0 and vice versa.</p>
				<xsl:for-each select="./mod:mod-version">
					<xsl:if test="substring(current(), 3, 1) mod 2 != 0 or substring(current(), 0, 1) = 0">
						<p><strong class="red">This MOD is development quality. It is not recommended that you install it on a live forum.</strong></p>
					</xsl:if>
				</xsl:for-each>
		</div>
		<hr />
	...
	<xsl:template name="give-sql">
		<div class="action">
			<h3><xsl:value-of select="@dbms" /></h3>
			<pre>
				<xsl:value-of select="current()" />
			</pre>
		</div>
